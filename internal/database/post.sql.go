...
const getPostDataForUser = `-- name: GetPostDataForUser :many
SELECT 
...
type GetPostDataForUserRow struct {
	ID          uuid.UUID
	CreatedAt   time.Time
	UpdatedAt   time.Time
	Title       sql.NullString
...
func (q *Queries) GetPostDataForUser(ctx context.Context, userID uuid.UUID) ([]GetPostDataForUserRow, error) {
	rows, err := q.db.QueryContext(ctx, getPostDataForUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetPostDataForUserRow
	for rows.Next() {
		var i GetPostDataForUserRow
		if err := rows.Scan(
			&i.ID,
			&i.CreatedAt,
			&i.UpdatedAt,
